...
## ビルド・実行コマンド
- iOS ビルド: `xcodebuild -scheme <name> build` (フル。重いので develop モードで毎回走らせない)
- iOS シミュレータ実行: Xcode から、または `xcrun simctl`
- Android ビルド: `./gradlew assembleDebug` (フル。同上)
- Android 実行: `./gradlew installDebug`
- 静的解析: `<コマンド>` (例: `swiftlint`, `./gradlew detekt`)

## 軽量セルフチェックコマンド (develop モードで毎サイクル走るもの)
- iOS: `<コマンド>` (例: `swift build` (Package.swift 構成) + `swiftlint`、または `xcodebuild -dry-run`)
- Android: `<コマンド>` (例: `./gradlew compileDebugKotlin` + `./gradlew detekt`。フル `assembleDebug` は避ける)
- 上記が空なら、フルビルドはスキップして言語の標準的な型チェック + リントを推定実行する
- iOS で Xcode プロジェクト (`.xcodeproj`) 構成の場合は CLI からの軽量チェックが難しいため、セルフチェック自体をスキップ可
