<!--
このファイルは /cc-development-team:develop の Step 5 (動作確認) で、以下の場合に Read します:
- Web: ユーザーが yes を選んだ時 (通常運用)
- Mobile: ユーザーが「CLI で起動して」と明示した時のみ (専用 IDE 起動を強く推奨するため通常は使わない)
プラットフォーム別の具体的な起動コマンドの参照表です。
-->

# 動作確認 — プラットフォーム別の CLI コマンド

> **Web は通常通り CLI で起動。Mobile は IDE 起動が推奨で、このファイルの Mobile 系コマンドは「ユーザーが CLI 起動を明示した場合のみ」使う最終手段です。**

ユーザーが起動を承認したら、以下のフォーマットで **最終確認** を取った上で、対応するプラットフォームのコマンドを実行する。
