...
| ドキュメント | 粒度 | 何を書く? |
| --- | --- | --- |
| 要件定義書 `docs/requirements/requirements.md` | プロジェクト全体 | 業務要件 / 機能要件 / 非機能要件 / スコープ |
| 基本設計書 `docs/basic-design/basic-design.md` | プロジェクト全体 | システム構成 / 画面設計 / データ設計 / API / 認証認可 |
| 機能仕様書 `docs/specs/<機能名>.md` | 機能ごと | 詳細仕様 / 受け入れ基準 (Given-When-Then) |

- 「なぜこう設計したか」の理由も残してくれるので、後から見返したときに分かりやすい
- 不明点があればあなたに質問してきます（曖昧なまま勝手に決めません）

**2 つのモードがあります:**
- **アプリ構想モード**: まだ作るものが決まっていない段階で呼ばれると、雑談ベースのヒアリングから vision.md → roadmap.md → **要件定義書** → **基本設計書** → MVP の先頭機能の仕様書 まで一気通貫で作成します。
- **機能設計モード**: 作るものが具体的に決まっている個別機能を、8 項目のヒアリングから仕様書に落とし込みます。既存の要件定義書・基本設計書を参照して整合性を保ちます。
...
[architect] 機能候補を MVP / Phase 2 / Future に分類 → roadmap.md 生成
   ↓ （MVP 全体で OK か確認）
[architect] 要件定義書を生成 → docs/requirements/requirements.md 確定
   ↓ （内容を確認 → OK 出す）
[architect] 基本設計書を生成 → docs/basic-design/basic-design.md 確定
   ↓ （内容を確認 → OK 出す）
...
**現在の挙動:** 1 回の `/cc-development-team:design` 実行で、`vision.md` → `roadmap.md` → `requirements.md`（要件定義書） → `basic-design.md`（基本設計書） → **MVP の最初の 1 機能の仕様書** まで作って停止します。実装には進みません。残りの MVP 機能を設計したい場合は、その都度 `/cc-development-team:design <feature-name>` を実行してください。

> **3 段階のドキュメント階層:**
> 1. 要件定義書 (`requirements.md`) — プロジェクト全体で「何を作るか・なぜ作るか」を明文化
> 2. 基本設計書 (`basic-design.md`) — プロジェクト全体で「どう作るか」を設計レベルで明文化
> 3. 機能仕様書 (`specs/<feature-name>.md`) — 個別機能の詳細（受け入れ基準まで）
>
> 各機能仕様書は要件定義書の機能 ID (F-XXX) と紐づきます。後から「これは何の機能だっけ?」「全体としてこの判断で合ってる?」を辿れる構造です。
...
│   ├── requirements/               # 要件定義書（プロジェクト全体）
│   │   └── requirements.md         # 業務要件 / 機能要件 / 非機能要件 / スコープ
│   ├── basic-design/               # 基本設計書（プロジェクト全体）
│   │   └── basic-design.md         # システム構成 / 画面 / データ / API / 認証認可
│   ├── specs/                      # 機能仕様書（機能ごとに1ファイル = 詳細設計）
│   │   ├── login-flow.md
│   │   ├── invoice-export.md
│   │   └── ...
