...
| アプリの構想から相談したい   | `/cc-development-team:feature`        | 引数なしで起動するとアプリ構想モードから始まる |
...
**2 つのモードがあります:**
- **アプリ構想モード**: まだ作るものが決まっていない段階で呼ばれると、雑談ベースのヒアリングから始めて vision.md と roadmap.md を作ります。**MVP に並べた全機能を順番にループ処理** し、1 セッションで一気に MVP を完成させます（途中で停止も可能）。
...
[architect] モード選択を確認（A: 構想 / B: 機能）
   ↓ (A を選択)
[architect] アプリ構想モードでヒアリング → vision.md 生成
   ↓
[architect] 機能候補を MVP / Phase 2 / Future に分類 → roadmap.md 生成
   ↓ （MVP 全体で OK か確認）
[architect] MVP の先頭機能の仕様書を作成 → docs/specs/<feature-1>.md 確定
   ↓
[developer → reviewer → tester]  feature-1 を実装・レビュー・テスト
   ↓
[feature.md] roadmap.md で feature-1 を [x] にマーク
   ↓
「次の feature-2 に進みますか? (yes/no/skip)」← ユーザーに確認
   ↓ (yes)
[architect] feature-2 の仕様書を作成 → developer → reviewer → tester → mark [x]
   ↓
「次の feature-3 に進みますか?」(以下、MVP 全件 or ユーザーが no と答えるまでループ)
   ↓
[feature.md] セッション完了報告（完了 / スキップ / 残機能 / 次のステップ提案）
```

**1 回の `/cc-development-team:feature`（引数なし）で MVP 全機能を一気に作り切れます。** 途中で `no` と答えれば一旦停止し、後で `/cc-development-team:feature <feature-name>` で個別に再開できます。
