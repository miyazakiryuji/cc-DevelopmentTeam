...
**2 つのモードがあります:**
- **アプリ構想モード**: まだ作るものが決まっていない段階で呼ばれると、雑談ベースのヒアリングから始めて vision.md と roadmap.md を作ります。MVP の最初の機能はそのまま機能設計モードに自動遷移します。
...
[architect] 機能候補を MVP / Phase 2 / Future に分類 → roadmap.md 生成
   ↓ （あなたが MVP の最初に作る機能を選択）
[architect] 機能設計モードに自動遷移 → docs/specs/<機能名>.md 確定
   ↓
（以降は通常の設計先行ルートと同じ：developer → reviewer → tester）
```
