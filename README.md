...
[あなた] 「ログイン機能作って」
   ↓
[developer → reviewer → architect (back-fill) → tester]  1 件目を処理
   ↓
[Claude] 「1 件完了。次の依頼は?」
   ↓
[あなた] 「次は検索機能作って」
   ↓
[developer → reviewer → architect (back-fill) → tester]  2 件目を処理
   ↓
[Claude] 「1 件完了。次の依頼は?」
   ↓ ...
[あなた] 「終了」
   ↓
[develop モード終了] セッション通算の報告（処理件数・変更ファイル・追加テスト 等）
```

**特徴:**
- 1 件ごとに `developer → reviewer → architect (back-fill) → tester` の 4 フェーズを実行
- 各サイクルの終わりに次の依頼を聞かれる
