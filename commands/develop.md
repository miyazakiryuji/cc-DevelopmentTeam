...
- 終了するには: 「終了」「終わり」「もう大丈夫」などと伝えてください
- 別モードに切り替えるには: `/cc-development-team:architect` などのスラッシュコマンドを実行（モード自動終了）
```

## サイクル範囲のヒアリング (`$CYCLE_MODE`)

続けて、1 サイクルでどこまで自動実行するかを必ずユーザーに尋ねる。**実装重視 (lean) が推奨デフォルト** です (フルサイクルは時間がかかるので、実装が一通り終わってからテストを別途回す方が効率的)。

```
【1 サイクルの範囲を選んでください】
番号で答えてくださいね!特にこだわりが無ければ a) で OK です〜

a) 実装重視 (推奨: 実装 → レビュー → 仕様書更新 → 動作確認)
   ※ tester は呼ばず実装に集中。1 サイクルが軽く、テンポよく回せます。
   ※ テストは実装が一通り終わってから /cc-development-team:test でまとめて実行する想定
b) フルサイクル (実装 → レビュー → テスト → 仕様書更新 → 動作確認)
   ※ tester も毎サイクル呼んでテストまで一気通貫。1 サイクルが重くなり、時間がかかります
```

ユーザーの回答を以下のように `$CYCLE_MODE` として保持:

- a を選択 → `$CYCLE_MODE = "lean"` (デフォルト推奨)
...
次の依頼はありますか?
- 次の機能 / 修正内容を教えてください!
- 終了する場合は「終了」「終わり」「もう大丈夫」などと伝えてくださいね
```
...
- 明示的な終了語句: 「終了」「終わり」「もう大丈夫」「やめる」「ストップ」「stop」
