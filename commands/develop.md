...
- 1 件の流れ:
    ...
    3) architect (仕様書の新規作成 or 更新 / 必要時のみ)
    4) 動作確認の起動 (Web/Mobile プロジェクトのみ、ユーザー承認のもと)
- 仕様書が既にある場合は developer がそれに従って実装します。
  仕様書が無い場合や、実装が仕様書を超えた場合は最後に仕様書を作成/更新します。
- **テストはこのモードでは実行しません**。完了後 `/cc-development-team:test` で別途実行してください。
...
### Step 4: モック起動の確認（Web / Mobile プロジェクトのみ）

実装したものを **動かして確認するか** をユーザーに尋ねる。

#### 4-1. プロジェクト種別と起動コマンドの判定
...
3. 変更したファイル一覧
4. **テスト状況**: 「このサイクルではテストを実行していません。`/cc-development-team:test $CURRENT_FEATURE` で実行できます」と必ず案内
5. **手動操作タスクの有無**:
   ...
   - 該当無しなら「手動タスク: 無し」と明記
6. **起動状態**:
   - Step 4 で起動した場合: アクセス先 (URL / シミュレータ名) と停止方法
...
4. **未実行のテスト**: 「テストは別途 `/cc-development-team:test` で実行してください」と案内
...
- **このモードで tester を起動する**（テストは `/cc-development-team:test` の責務。混ぜないこと）
- 「仕様書外で追加した内容」があるのに Step 3 をスキップ
- 「仕様書外で追加した内容」が無いのに architect を呼ぶ（無駄な呼び出し）
- **Step 4 の起動確認をユーザーに聞かずにスキップする**（CLI/ライブラリ等で起動対象が無い場合のみスキップ可）
