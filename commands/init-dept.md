...
**Step 0 のウェルカム説明は必ず先頭で出力する**こと（飛ばさない）。

## 0. ウェルカム説明（最初に必ず出力）

以下のテキストを **そのまま整形して** ユーザーに表示してください。表の崩れに気を付けて、コードブロックや表 (`|---|` 形式) は Markdown としてそのまま出すこと。
...
ざっくり言うと、以下の 4 部署を順番に呼び出して開発を進めます:
...
| 部署 | 担当 | 何をしてくれる人? |
| --- | --- | --- |
| security-reviewer | セキュリティ専門アドバイザー | OWASP 観点 / 秘密情報漏れ / 認可漏れ / 依存脆弱性 など、セキュリティに集中して点検する。普段は黙っているが、認証・決済・個人情報を扱った後や、リリース前点検で呼ぶと頼れる |
...
それでは、まずあなたのプロジェクト用に初期化を進めていきますね。
最初に簡単なヒアリングだけさせてください ↓
```

ウェルカム説明を出し終えたら、Step 1 のプロジェクト種別ヒアリングに進んでください。
