...
A. `develop` モード開始時に **「実装重視 (推奨) / フルサイクル」をヒアリング** します。

- **実装重視 (lean、推奨)**: tester は呼ばず実装に集中。1 サイクルが軽くテンポよく回せる。テストは実装が一通り終わってから `/cc-development-team:test` でまとめて実行する想定
- **フルサイクル (full)**: 毎サイクル tester も呼ばれ、テスト作成 + 全体実行まで自動で進む。1 サイクルが重くなり時間がかかるので、慎重に進めたい時向け

デフォルトは **lean (推奨)** です。「実装が落ち着いてから一気にテストする」運用の方が、開発のテンポも保てて、テスト失敗時の原因も切り分けやすいです。途中で「フルに切り替えて」「軽量で」と伝えれば変更できます。失敗テストや改善候補が見つかった場合は、**修正前に必ずユーザーに確認** するので勝手にコードを書き換えません。
