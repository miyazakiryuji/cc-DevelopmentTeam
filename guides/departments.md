...
- **ユーザー主体・推測禁止**: ヒアリングで明示されていない情報を推測で書き加えません。不足分があれば章を書く前にユーザーに質問します (機能 / エンティティ / 技術選定 / ケース 等)。技術判断は必ず選択肢を 2〜3 個提示してユーザーに選んでもらいます。

> `/cc-development-team:architect` は **設計フェーズのみ** で停止します。実装には自動では進みません。実装に進める場合は `/cc-development-team:develop` を別途実行してください。
