...
1. **プロジェクト固有メモを読む**: `docs/dept/developer/CLAUDE.md` を `Read` してコーディング規約・実装パターン・ビルドコマンドを把握する。ファイルが無ければユーザーに `/cc-development-team:init-dept` の実行を案内し、メモが整備されるまでは既存コードのスタイルから推定して進める。
